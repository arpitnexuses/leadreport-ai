...
// Dynamic imports for Puppeteer to avoid build issues
// process.env.AWS_LAMBDA_FUNCTION_VERSION is a good indicator of Vercel/AWS env
...
async function generateSimplePDF(report: any): Promise<Buffer> {
  ...
  doc.text(`${leadData.name} - Shared Report`, 20, yPos);
  yPos += 15;
  ...
  if (report.reportOwnerName || report.email) {
    doc.text(`Shared by: ${report.reportOwnerName || report.email.split('@')[0]}`, 20, yPos);
    yPos += 8;
  }
  yPos += 10;
...
async function generatePuppeteerPDF(reportId: string): Promise<Buffer | null> {
  ...
  try {
    console.log('Starting Puppeteer PDF generation for shared report');
    console.log('Environment:', process.env.NODE_ENV, 'Is Production:', IS_PRODUCTION);

    // Launch browser based on environment
    if (IS_PRODUCTION) {
      console.log('Using Production Puppeteer Core + Chromium');
      const chromium = require('@sparticuz/chromium');
      const puppeteer = require('puppeteer-core');

      // Optional: Load specific font if needed, otherwise chromium uses default
    ...
    // Set viewport to A4 equivalent (approx) to ensure responsive breakpoints hit correctly
    // A4 at 96 DPI is ~794px width. But we want to trigger desktop styles, so we use 1200px.
    // We will scale it down in the PDF print options.
    await page.setViewport({
      width: 1200,
      height: 1600,
      deviceScaleFactor: 2 // Higher quality for images/text
    });
    ...
    // Pass a secret or query param if needed to bypass auth or enable print mode
    ...
    // Prepare page for printing - Inject Print CSS
    await page.addStyleTag({
      content: `
        ...
        * {
        ...
        /* Handle responsive classes - force them to desktop view */
        ...
        p, li, span, div { font-size: 11px !important; line-height: 1.5 !important; }
        ...
        /* Ensure links are readable and not blue/underlined if not needed */
        a { text-decoration: none !important; color: inherit !important; }
      `
    });

    // Force expand all details/accordions
    await page.evaluate(() => {
      document.querySelectorAll('[data-state="closed"]').forEach(el => {
        el.setAttribute('data-state', 'open');
      });
      // Fallback for details element
    ...
    const pdfBuffer = await page.pdf({
      format: 'A4',
      landscape: true, // Lead reports usually look better in landscape given the 3-column layout? Or user prefers Portrait?
      // The existing code had landscape: true in the commented out A4 section, but page.setViewport width 1400.
      // 3-column layouts usually work better in Landscape or scaled down Portrait.
      // Let's stick to Landscape as it matches the screen layout better.
      ...
      scale: 0.8 // Slight scale down to fit more content
    });
...
export async function GET(
  ...
  try {
    ...
    console.log(`PDF generation started for shared report: ${id}`);
    ...
    if (puppeteerPdf) {
      ...
      return new NextResponse(puppeteerPdf as any, {
        headers: {
    ...
    return new NextResponse(jspdfBuffer as any, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${report.leadData.name.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-shared-report.pdf"`,
      },
